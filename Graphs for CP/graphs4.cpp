/*
Q >> You are given a map of a labyrinth, and your task is to find a path from start to end.
     You can walk left, right, up and down.

    First print "YES", if there is a path, and "NO" otherwise.
    If there is a path, print the length of the shortest such path.
    Give its description as a string consisting of characters L (left), R (right), U (up), and D (down).
    ...
    For printing path, we keep track of its parent
*/
...
int main()
{
    ...
    for (int i = 1; i <= dist[ex][ey]; i++) printf("%c", ans[i]);
    printf("\n");
}